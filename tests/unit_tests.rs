...
#[test]
fn profile_default_denies_dcp_exec() {
    let p = generate_profile(&ProfileOptions {
        project_dir: std::path::Path::new("/projects/app"),
        home_dir: std::path::Path::new("/Users/test"),
        extra_read: &[],
    ...
    assert!(
        !p.contains("aspire"),
        "Default profile must NOT contain Aspire/DCP rules — opt-in via --allow-dcp"
    );
    // The broad .nuget exec deny (binary-drop protection) must still be present.
    assert!(
        p.contains("(deny process-exec (subpath \"/Users/test/.nuget\"))"),
        "Profile must still deny broad process-exec under ~/.nuget by default"
    );
}

#[test]
fn profile_allows_dcp_when_flag_set() {
    let home = "/Users/test";
    let p = generate_profile(&ProfileOptions {
        project_dir: std::path::Path::new("/projects/app"),
        home_dir: std::path::Path::new(home),
        extra_read: &[],
        extra_write: &[],
        allow_socket: &[],
        extra_deny: &[],
    ...
    let dcp_pattern =
        "^/Users/test/\\.nuget/packages/aspire\\.hosting\\.orchestration\\.[^/]+/[^/]+/tools/dcp$";
    let exec_allow = p
        .find(&format!("(allow process-exec (regex #\"{dcp_pattern}\"))"))
        .expect("must allow process-exec for the dcp binary pattern under ~/.nuget/packages");
    let write_deny = p
        .find(&format!("(deny file-write* (regex #\"{dcp_pattern}\"))"))
        .expect("must deny file-write* for the dcp binary pattern (read-only carve-out)");
    let broad_deny = p
        .find("(deny process-exec (subpath \"/Users/test/.nuget\"))")
        .expect("broad ~/.nuget exec deny must still be present");
    assert!(
        exec_allow > broad_deny,
        "The narrow dcp exec allow must come after (override) the broad ~/.nuget exec deny"
    );
    assert!(write_deny > exec_allow, "sanity: deny emitted after allow");
    // No custom NUGET_PACKAGES root was given — must not emit that section.
    assert!(
...
#[test]
fn profile_allows_dcp_with_custom_nuget_packages() {
    let nuget_packages = "/opt/custom-nuget/packages";
    let p = generate_profile(&ProfileOptions {
        project_dir: std::path::Path::new("/projects/app"),
        home_dir: std::path::Path::new("/Users/test"),
        extra_read: &[],
        ...
        copilot_install_dir: None,
        java_home: None,
        nuget_packages: Some(std::path::Path::new(nuget_packages)),
        dotnet_root: None,
        git_hooks_path: None,
        git_common_dir: None,
        allow_gpg_signing: false,
    ...
    assert!(
        p.contains(&format!(
            "(allow file-read* (subpath \"{nuget_packages}\"))"
        )),
        "Custom NUGET_PACKAGES root must be readable"
    );
    assert!(
        p.contains(&format!(
            "(allow file-map-executable (subpath \"{nuget_packages}\"))"
        )),
        "Custom NUGET_PACKAGES root must allow dylib loading"
    );
    let custom_pattern =
        "^/opt/custom-nuget/packages/aspire\\.hosting\\.orchestration\\.[^/]+/[^/]+/tools/dcp$";
    assert!(
        p.contains(&format!(
            "(allow process-exec (regex #\"{custom_pattern}\"))"
        )),
        "Must allow process-exec for the dcp binary under the custom NUGET_PACKAGES root"
    );
    assert!(
        p.contains(&format!("(deny file-write* (regex #\"{custom_pattern}\"))")),
        "Must deny writes to the dcp binary under the custom NUGET_PACKAGES root"
    );
    // The default ~/.nuget/packages carve-out must still be emitted alongside it.
    assert!(
        p.contains(
            "^/Users/test/\\.nuget/packages/aspire\\.hosting\\.orchestration\\.[^/]+/[^/]+/tools/dcp$"
        ),
        "Default home-based dcp carve-out must still be present alongside a custom root"
    );
}
